...
t1<- grid::textGrob(
             expression("Overall, is the economy getting better or worse, for those that" * phantom(bold("Voted")) *" and " * phantom(bold("Did not vote")) * " ?" ), gp = grid::gpar(col = "black"))
                                                                                                                      
t2<- grid::textGrob(
             expression(phantom("Overall, is the economy getting better or worse, for those that") * (bold("Voted") *" and " * phantom(bold("Did not vote")) * " ?" ), gp = grid::gpar(col = "#440154FF")))

t3<- grid::textGrob(
             expression(phantom("Overall, is the economy getting better or worse, for those that") * phantom(bold("Voted") *" and " * bold("Did not vote") * " ?" ), gp = grid::gpar(col = "##FDE725FF")))
                                                                                                                                       

my_title<-expression(paste("A ", italic(bold("green")), " histogram"))

voters %>%
  ggplot(aes(econtrend_2016, ..density.., fill = turnout16_2016))+
  geom_histogram(alpha = 0.5, position = "identity", binwidth = 1)+
  labs( caption = str_c("Jim Gruman, ", Sys.Date()))+
  theme(legend.position = "top" # ,
     #   plot.title.position = "plot"
     )+     #tip from William R Chase at RStudio Conference 2020
  scale_fill_viridis(discrete = TRUE)+
  annotation_custom(grid::grobTree(t1,t2,t3))
