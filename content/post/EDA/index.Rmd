...
It's common to want to explore the distribution of a continuous variable broken down by a categorical variable, as in the previous frequency polygon. The default appearance of `geom_freqpoly()` is not that useful for that sort of comparison because the height is given by the count. To make the comparison easier we need to swap what is displayed on the y-axis. Instead of displaying count, we'll first display __density__, which is the count standardized so that the area under each frequency polygon is one.
...
So, in this case frequency polygons may be a little hard for most audiences to interpret. There's a lot going on in this plot.
...
For small datasets, an combination `GGally` visualization can be built to illustrate pairwise relationships. Unfortunately, the graphic that looks reasonable with only three variables is not so useful with more, particularly for non-techincal audiences.
