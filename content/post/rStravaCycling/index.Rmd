...
all_activity_heatmap <- get_heat_map(act_data, key = Sys.getenv("GGMAP_GOOGLE_API_KEY"), col = pal[4], size = 2, distlab = F, f = 0.4) +  
  ...
  labs(title = "My Cumulative Exercise Activity in Strava", 
       subtitle = paste0(format(as.Date(min(act_data$start_date)),"%B %e, %Y")," to ", format(today(),"%B %e, %Y")),
       fill = "", x = NULL, y = NULL,
       caption = "map: Google") +
...
`goal`: (num) - The goal in miles to achieve for each activity. For here: 900 
...
p <- act_data %>% 
  ...
  labs(x = NULL, y = "miles completed", 
       title = paste0('{format(frame_along,"%B %d")}'," progress towards ",selected.year, " goals in Strava"),
       caption = "@jim_gruman") 
