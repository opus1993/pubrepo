...
lims <- c(max(covus$date)-30, max(covus$date))
...
covus %>%
  ...
  theme(plot.title.position = "plot",
        plot.margin = unit(c(c(0.5, 0.5, 0.5, 0.5)), units="cm"))
...
## Timespan
lims <- c(max(nytcovcounty$date)-45, max(nytcovcounty$date))
...
IllinoisCountyPlot<-IllinoisCounty %>%
...
direct_labels <- IllinoisCounty %>%
  summarize(y.label = last(NewCases, order_by = date)) %>%
  top_n(3)

direct_labels_axis <- cowplot::axis_canvas(IllinoisCountyPlot, axis = "y")+
  geom_text(
    data = direct_labels,
    aes(y = y.label, label = county, color = county),
    x = -.1,
    hjust = 0,
    size = 4
  )+
...
IllinoisCountyplotLabeled<-cowplot::insert_yaxis_grob(IllinoisCountyPlot, direct_labels_axis, position = "right")

cowplot::ggdraw(IllinoisCountyplotLabeled)
...
```{r IowaCasesByCounty}
## Counties to highlight
focus_ct <- c("Johnson", "Woodbury", "Linn", "Polk", "Black Hawk", "Marshall", "Muscatine")
...
## Colors
cgroup_cols <- c(clr_darken(paletteer_d("ggsci::category20_d3"), 0.2)[1:length(focus_ct)], "gray70")

## Timespan
lims <- c(max(nytcovcounty$date)-45, max(nytcovcounty$date))

IowaCounty<-nytcovcounty %>%
...
IowaCountyplot<-IowaCounty %>%
  ggplot(aes(x = date, y = NewCases, group = county, color = core)) +
  geom_point(size = 2, shape =1) +
  geom_smooth(size = 0.5, se = FALSE , method = "loess")+
  guides(color = FALSE) +
  scale_color_manual(values = cgroup_cols) +
...
direct_labels <- IowaCounty %>%
  summarize(y.label = last(NewCases, order_by = date)) %>%
  top_n(7)

direct_labels_axis <- cowplot::axis_canvas(IowaCountyplot, axis = "y")+
  geom_text(
    data = direct_labels,
    aes(y = y.label, label = county, color = county),
    x = -.1,
...
IowaCountyplotLabeled<-cowplot::insert_yaxis_grob(IowaCountyplot, direct_labels_axis, position = "right")

cowplot::ggdraw(IowaCountyplotLabeled)
