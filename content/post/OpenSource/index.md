...
Had the company been using open source software, the file format would have either remained unchanged between software versions or the ability to import the older files would have been provided. A great example of this is found with Microsoft Office documents. Even though today's LibreOffice has fair docx support, it's still known to cause formatting and other compatibility issues due to Microsoft's Word's previously closed document standard. With LibreOffice's support of the ODF format, Word 2007 and higher all support the file format just fine. After all, it's an open document standard. So adopting it into an application is transparent. The same can't be said for the legacy docx.
